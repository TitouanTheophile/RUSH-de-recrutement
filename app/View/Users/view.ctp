...
<div id="wall_infos" class="container_padding">
		...
		<div class="container_padding">
			<?php 
				if (!empty($user['User']['study_place']))
					echo "<p>Étudie à <strong>" . $user['User']['study_place'] . "</strong></p>";
				if (!empty($user['User']['work_place']))
				...
				if (!empty($user['User']['birthday']))
					echo "<p>Né le : <strong>" . $this->Time->format($user['User']['birthday'], '%d %B %G') . "</strong></p>";
				if (!empty($user['User']['created']))
					echo "<p>Inscrit depuis le <strong>" . $this->Time->format($user['User']['created'], '%d %B %G') . "</strong></p>";
			?>
		</div>
		<table>
